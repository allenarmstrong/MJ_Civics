...
$(document).ready(function() {
	...
	function init(innerDirections, container, count, correct, total, words, ids, definitions) {
		...
		$('.t3565').append('<h1 id="title1j">Amendment</h1><h1 id="title2j">Title</h1>');
		...
		$('.word, .definition').click(function() {
			...
			if (count == 2) {
				...
				if (answers[0] == answers[1]) {
					...
					correct ++;
					if (correct == total) {
						$(container).append('<div id="instructions"><div id="title">Great job!</div><div id="text">If you would like to try this again, please select the Continue button.</div><div id="go-btn">Continue</div></div>');
						$('#go-btn').click(function() {
							$('#instructions').fadeOut('slow', function() {
								$(this).remove();
